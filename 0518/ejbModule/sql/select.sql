...
select empno, ename, job , hiredate,deptnofrom from emp;

select ename as 사원명, job as 업무,sal as 급여,comm as 수당 from emp;

select ename 사원명, job 업무,sal 급여,comm 수당 from emp; 
...
select ename,nvl(to_char(mgr),'없음') as manager from emp;

select ename,mgr,'sql' from emp;

select ename,sal,comm,sal+nvl(comm,0)as 지급액 as '원' from emp;
